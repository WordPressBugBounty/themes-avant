 ...
 * @package Avant
 */
define( 'AVANT_THEME_VERSION' , '1.2.01' );
...
function avant_scripts() {
	...
	wp_enqueue_style( 'avant-font-awesome', get_template_directory_uri().'/includes/font-awesome/css/all.min.css', array(), '6.5.1' );
...
function avant_register_required_plugins() {
	$plugins = array(
		...
		array(
			'name'      => __( 'Blockons (WordPress Editor Blocks)', 'avant' ),
			'slug'      => 'blockons',
			'required'  => false,
		),
		array(
...
function avant_add_license_notice() {
	...
	if ( !get_user_meta( $avant_user_id, 'avant_admin_notice_ignore' ) ) : ?>
		<div class="notice notice-info avant-admin-notice avant-notice-add">
            ...
            <p><?php printf( __( 'We pride ourselves on <a href="%1$s" target="_blank">good products and 5 Star Support</a>! Please read through our <a href="%2$s">About Avant</a> page for more help on using the Avant theme, or to get theme support', 'avant' ), 'https://wordpress.org/support/theme/avant/reviews/?filter=5', admin_url( 'themes.php?page=avant_theme_info' ) ); ?></p>
			
			<?php if ( $avantpage == 'themes.php?page=avant_theme_info' ) : ?>
				<h5>
					<?php
					/* translators: %s: 'Recommended Resources' */
					printf( esc_html__( 'Avant Premium is %1$s for all the extra %2$s plus FREE shortcode slider plugin included', 'avant' ), wp_kses( __( '<a href="https://kaira.lemonsqueezy.com/checkout/buy/f40e3470-5679-403a-9bae-4036a3af121c" target="_blank">currently selling for only $25</a>', 'avant' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ), wp_kses( __( '<a href="https://kairaweb.com/wordpress-theme/avant/#premium-features" target="_blank">Avant Premium features</a>', 'avant' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ) );
					?>
				</h5>
 ...
 */
function avant_blockons_notice() {
	global $current_user;
	$avant_user_id = $current_user->ID;

	if ( !get_user_meta( $avant_user_id, 'avant_blockons_dismiss' ) ) : ?>
		<div class="notice notice-info avant-admin-notice avant-notice-blockons">
			<div>
			...
			<div>
				<h4><?php esc_html_e( 'Try out the new Blockons Plugin !', 'avant' ); ?></h4>
				<p><?php esc_html_e( 'Blockons offers advanced WordPress blocks for your Editor, as well as Site Addons such as page loader, page scroll indicator & back to top button... Great for building beautiful pages! More features coming soon!', 'avant' ); ?></p>
				<a href="<?php echo esc_url(admin_url('/plugin-install.php?s=blockons&tab=search&type=term')); ?>"><?php esc_html_e( 'View the Blockons plugin', 'avant' ); ?></a>
			</div>
			<a href="?avant_blockons_notice_ignore=" class="avant-notice-close"><?php esc_html_e( 'Dismiss Notice', 'avant' ); ?></a>
		</div><?php
	endif;
}
add_action( 'admin_notices', 'avant_blockons_notice' );
/**
 * Admin notice dismiss for Blockons
 */
function avant_blockons_notice_ignore() {
    global $current_user;
	$avant_user_id = $current_user->ID;

    if ( isset( $_GET['avant_blockons_notice_ignore'] ) ) {
		update_user_meta( $avant_user_id, 'avant_blockons_dismiss', true );
    }
}
add_action( 'admin_init', 'avant_blockons_notice_ignore' );